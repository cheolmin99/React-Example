...
//? .tsx 파일 내부에서 'rfc'로 자동 함수형 컴포넌트를 자동 생성
...
// let number = 0;
//? state를 선언하는 방법
//? const [상태명(변수명), 상태변경메서드(set메서드)] = useState<>(초기값);
     ...
     <div className="inline">
      {
        ...
        //? input의 값이 바뀌었을 때 동작을 지정하고자 할때는
        //? html의 onChange 속성을 사용
        //? onChange{함수}
      }
      { inputValue }
       <input className="input-style" type={type} onChange={(event) => 
          setInputValue(event.target.value)
       }/>
     </div>
     
     { //# 클릭에 대한 이벤트 처리 
       //? 클릭했을때의 동작을 지정하고자 할때는 
